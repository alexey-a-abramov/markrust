...
- **Highlight / sub / sup:** Typora `==highlight==` (comrak has no node; applied on import) and HTML `<mark>` paint a background. Comrak `^sup^` / `~sub~` and HTML `<sub>`/`<sup>` map through Unicode super/subscripts. GPUI `TextRun` has no per-run font size or baseline offset, so unmapped letters stay the same size.
...
- HTML `<svg>` / `style=` / CSS classes are not painted. GPUI `TextRun` has no per-run baseline or font size, so `<sub>`/`<sup>` / `~` / `^` use a Unicode map (unmapped letters stay body size).
