...
- Remote `http(s)` images still go through GPUI URI loading (network); missing local files fall back to an alt placeholder. Mixed paragraphs are a flex line-box, not CSS `display:inline` wrap-around inside one shaped line — a long text run still wraps as a unit rather than flowing around the image mid-line.
- Mermaid WYSIWYG is not started (fenced `mermaid` stays an inert code block).
