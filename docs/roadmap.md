...
- `crates/markrust-core/src/html_visual.rs` — safe HTML / footnote / definition-list paint projection (no script, no CSS).
- `crates/markrust-core/src/html_visual.rs` — safe HTML / footnote / definition-list paint projection (no script, no CSS).
...
- **Safe HTML paint:** inline tags (`<b>`, `<i>`, `<code>`, `<kbd>`, `<mark>`, `<br>`, `<a>`, …) are hidden chrome; inner text keeps Markdown marks plus HTML phrasing. HTML blocks strip tags and show inner text (or a rule / `<img>`). `script` / `iframe` / `javascript:` URLs are dropped, not executed. Caret skips tag bytes like `**` delimiters.
- **Footnotes / definition lists:** comrak extensions on (Typora extras, not GFM). `[^1]` paints as a superscript; `[^1]:` defs and `term` / `: details` are styled opaque blocks. Nested Markdown inside those opaque blobs is not re-parsed.
...
- Footnote / definition-list *bodies* are opaque: inner `**bold**` is not a nested rich tree. HTML `<svg>` / `style=` / CSS classes are not painted. `<sub>`/`<sup>` use Unicode super/subscripts when every character maps; otherwise same-size glyphs.
