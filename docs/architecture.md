...
WYSIWYG: virtualized block list + BlockTextElement; local images as `img(PathBuf)` (GPUI background decode); IME origin from the focused widget or caret leaf (`wysiwyg/ime.rs`), pushed with `invalidate_character_coordinates` after caret/widget paint
```
